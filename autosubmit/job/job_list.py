...
class JobList:
    ...
    def _delete_edgeless_jobs(self):
        ...
        for job in self._job_list[:]:
            if job.dependencies is not None and job.dependencies not in ["{}", "[]"]:
                ...
                job.has_children()) and str(job.delete_when_edgeless).casefold() == "true".casefold()):
    ...
    def generate(self, as_conf, date_list, member_list, num_chunks, chunk_ini, parameters,
                 date_format, default_retrials, default_job_type, wrapper_jobs=None, new=True,
                 run_only_members=None, show_log=True, force=False, create=False):
        """
        ...
        :type show_log: bool
        """
        if wrapper_jobs is None:
            wrapper_jobs = {}
        if run_only_members is None:
            run_only_members= []
        ...
        # This if allows to have jobs with dependencies set to themselves even if
        # there are no more than one chunk, member or split.
    ...
    def _deep_map_dependencies(self, section, jobs_data, option, dependency_list=None,
                               strip_keys=True):
        """
    ...
    @staticmethod
    def _parse_filters_to_check(list_of_values_to_check, value_list=None, level_to_check="DATES_FROM"):
    ...
    @staticmethod
    def _parse_filter_to_check(value_to_check, value_list=None, level_to_check="DATES_FROM",
                               splits=None) -> list:
    ...
    def get_submitted(self, platform: Optional['Platform'] = None, hold=False, wrapper=False) -> list[Job]:
        ...
        if hold:
            submitted = [
                ...
                for job in self._job_list
                if (
                               platform is None or job.platform.name == platform.name) and job.status == Status.SUBMITTED and job.hold == hold
            ]
        else:
            submitted = [
                job
    ...
    def get_ready(self, platform: Optional['Platform'] = None, hold=False, wrapper=False) -> list[Job]:
        ...
        ready = [
            job
            for job in self._job_list
            if (platform is None or platform == "" or job.platform.name == platform.name) and
               job.status == Status.READY and job.hold == hold
        ]
        if wrapper:
    ...
    def update_list(self, as_conf: AutosubmitConfig, store_change: bool = True,
                    from_set_status: bool = False, first_time: bool = False) -> bool:
        """
...
def load_job_list(
    ...
    # TODO: If we load a job list with submitted statuses, should we reset then here? Or alert the use
