        ...
        for section in jobs_to_submit_by_section:
            ...
            if wrapper_defined and self._platform.allow_wrappers and self.wrapper_type[self.current_wrapper_section] in ['horizontal', 'vertical','vertical-horizontal', 'horizontal-vertical'] :
                ...
                for p in built_packages_tmp:
                    ...
                    if len(p.jobs) > 0:
                        if failed_innerjobs and self.wrapper_policy[self.current_wrapper_section] == "mixed":
                            for job in p.jobs:
                                if job.fail_count == 0:
                                    continue
                                    ...
                                    if self.wrapper_policy[self.current_wrapper_section] == "strict":
                                        ...
                                        for job in p.jobs:
                                            ...
                                            if len(active_jobs) > 0:
                                                if show_log:
                                                    Log.printlog(
                                                        "Wrapper policy is set to STRICT and there are not enough jobs to form a wrapper. [wrappeable_h:{0} < defined_min_h:{1}]|[wrappeable_v:{2} < defined_min_v:{3}] waiting until the wrapper can be formed.".format(
