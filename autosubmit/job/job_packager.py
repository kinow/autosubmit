...
class JobPackager(object):
    ...
    def calculate_wrapper_bounds(self, section_list: list[str]) -> dict[str, Union[dict, int]]:
        ...
        :return: Dictionary with keys: min, max, min_v, max_v, min_h, max_h, max_by_section
        """
    ...
    # TODO: Add type to this function some day; I started adding but it became quite a mess. It may require some
    #       time to focus, and patience...
    def check_packages_respect_wrapper_policy(self, built_packages_tmp, packages_to_submit, max_jobs_to_submit,
        ...
        """Check if the packages respect the wrapper policy.

        Act in base of it ( submit wrapper, submit sequential, wait for more jobs to form a wrapper).
        ...
        for p in built_packages_tmp:
            ...
            # Check if the user is using the option to run first some jobs.
            # if so, remove non-first jobs from the package and submit them
    ...
    def handle_mixed_policy(self, p: JobPackageBase, packages_to_submit: list, max_jobs_to_submit: int,
                            err_message) -> int:
        """Handle the mixed policy case by submitting failed jobs sequentially
        or/and filling ``self.wrappers_with_error`` with the error message.
    ...
    def check_if_packages_are_ready_to_build(self) -> tuple[list[Job], bool]:
        ...
        if self.hold and jobs_ready:
            ...
            jobs_in_held_status = chain(self._jobs_list.get_held_jobs(),
    ...
    def build_packages(self) -> list[JobPackageBase]:
        ...
        for wrapper_name, jobs in jobs_to_wrap.items():
            ...
            # FIXME: Is it correct? A list with params, and then we append the AutosubmitConfig object?
        ...
        for job in section_list:
            if wrapper_limits["max"] > 0:
                if not job.packed_during_building:
                    ...
                    job_vertical_packager = JobPackagerVerticalMixed(
                        dict_jobs, job, [job], job.wallclock, wrapper_limits["max"], wrapper_limits,
                        self._platform.max_wallclock, wrapper_info=wrapper_info)
                    jobs_list = job_vertical_packager.build_vertical_package(job, wrapper_info)
                    job_package_vertical = JobPackageVertical(jobs_list, configuration=self._as_config,
                                       wrapper_section=self.current_wrapper_section, wrapper_info=wrapper_info)
                    packages.append(job_package_vertical)
            else:
                break
        return packages
...
class JobPackagerVertical(object):
    ...
    def build_vertical_package(self, job, wrapper_info) -> list[Job]:
        """Build a vertical job package.
    ...
    def build_horizontal_package(self, horizontal_vertical=False, wrapper_info: Optional[list] = None):
        ...
        for section in jobs_by_section:
            ...
            for job in jobs_by_section[section]:
                ...
                if (
                        len(current_package) < self.wrapper_limits["max_h"] and
                        len(current_package) < self.wrapper_limits["max"] and
                        current_package_by_section[section] < self.wrapper_limits["max_by_section"][section]
                ):
