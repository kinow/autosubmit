...
class TestSagaPlatform(TestCase):
    def setUp(self):
        ...
        self.platform = SagaPlatform(
            self.experiment_id, 'test', FakeBasicConfig)
    ...
    def test_check_status_returns_the_right_states(self):
        ...
        self.platform.service.get_job = Mock(side_effect=[FakeJob('any-name', saga.job.UNKNOWN),
                                                          FakeJob(
                                                              'any-name', saga.job.PENDING),
                                                          FakeJob(
                                                              'any-name', saga.job.FAILED),
                                                          FakeJob(
                                                              'any-name', saga.job.CANCELED),
                                                          FakeJob(
                                                              'any-name', saga.job.DONE),
                                                          FakeJob(
                                                              'any-name', saga.job.RUNNING),
    ...
    def test_creates_a_saga_job_correctly(self):
        ...
        self.platform.add_attribute.assert_any_call(
            jd, 'Queue', parameters["CURRENT_QUEUE"])
        ...
        self.platform.add_attribute.assert_any_call(
            jd, 'TotalCPUCount', parameters["NUMPROC"])
        self.platform.add_attribute.assert_any_call(
            jd, 'ProcessesPerHost', parameters["NUMTASK"])
        self.platform.add_attribute.assert_any_call(
            jd, 'ThreadsPerProcess', parameters["NUMTHREADS"])
        self.platform.add_attribute.assert_any_call(
            jd, 'TotalPhysicalMemory', parameters["MEMORY"])
    ...
    def test_deleting_file_on_non_local_platform_makes_the_right_call(self):
        ...
        self.assertTrue(deleted)
        sys.modules['saga'].filesystem.File.assert_called_once_with(
            "sftp://{0}{1}".format(self.platform.host,
                                   os.path.join(self.platform.get_files_path(), 'file/path')),
