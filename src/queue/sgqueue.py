...
class SgQueue(HPCQueue):
	def __init__(self, expid):
		self._host = "stargate"
		self._scratch = "/scratch"
		self._project = "cfu"
		self._user = "dmanubens"
		self._expid = expid
		self._remote_log_dir = self._scratch + "/" + self._project + "/" + self._user + "/" + self._expid + "/LOG_" + self._expid
		self._cancel_cmd = "ssh " + self._host + " scancel"
		self._checkjob_cmd = "ssh " + self._host + " sacct -j"
		self._checkhost_cmd = "ssh " + self._host + " echo 1"
		self._submit_cmd = "ssh " + self._host + " sbatch -D " + self._remote_log_dir + " " + self._remote_log_dir + "/"
		self._put_cmd = "scp"
		self._get_cmd = "scp"
		...
		self._job_status['QUEUING'] = ['PENDING', 'CONFIGURING', 'RESIZING']
		self._job_status['FAILED'] = ['FAILED', 'CANCELLED', 'NODE_FAIL', 'PREEMPTED', 'SUSPENDED', 'TIMEOUT']
