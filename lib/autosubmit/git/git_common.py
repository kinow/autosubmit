			...
			for dirname in dirs:
				...
				if path.isdir(LOCAL_ROOT_DIR + "/" + self._expid + "/" + LOCAL_GIT_DIR + "/" + dirname):
					if path.isdir(LOCAL_ROOT_DIR + "/" + self._expid + "/" + LOCAL_GIT_DIR + "/" + dirname + "/.git"):
						(status, output) = getstatusoutput("cd " + LOCAL_ROOT_DIR + "/" + self._expid + "/" + LOCAL_GIT_DIR + "/" + dirname + "; " + "git diff-index HEAD --")
						if (status == 0):
							if (output):
								...
								(status, output) = getstatusoutput("cd " + LOCAL_ROOT_DIR + "/" + self._expid + "/" + LOCAL_GIT_DIR + "/" + dirname + "; " + "git log --branches --not --remotes")
								if (output):
									...
									(status, output) = getstatusoutput("cd " + LOCAL_ROOT_DIR + "/" + self._expid + "/" + LOCAL_GIT_DIR + "/" + "; " + "git clone --bare " + dirname + " " + dirname + ".git")
