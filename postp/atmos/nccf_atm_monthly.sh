...
####  process each variable  ####
 for VAR in ${VAR_LST_2D[@]}; do # untar once and extract all the variables
	new_name $VAR	
      if [ "$VAR" = "PRECIP" ]; then # for precip, have to add CP and LSP to get total precip
	varnew=prlr
        for f in ${files}; do
          suffix=$mem.$f
	...
	new_name $VAR       
       ...
       case ${varnew} in
       "rss"|"rls"|"hflsd"|"hfssd"|"rsds"|"rlt"|"rst"|"rltcs"|"rstcs")
...
for var in ${VAR_LST}; do
 ...
 case $var in
 T2M|SST|MSL|PRECIP|SSR|STR|TTR|TSR|TSRC|TTRC|SLHF|SSHF|U10M|V10M|SSRD)
 VAR_LST_2D[$j]="$var"
 j=$((j+1))
 ;;
 T|U|V|Z|Q)
